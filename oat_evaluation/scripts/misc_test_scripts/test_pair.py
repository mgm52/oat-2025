...
if __name__ == "__main__":
    ...
    gemma_llm = AutoLLM(config["MODEL_PATHS"]["llama"]["path"], dtype=torch.bfloat16, debug_mode=False)
    ...
    with torch.no_grad():
        for attack in attacks_to_try:
            ...
            print(f"Ultimately, generated prompt: {attack_details.generated_str_prompts}")
            print(f"Response after attack: {response_after_attack}")
            print(f"Attack flop cost: {attack_details.flop_cost}")
            print(f"Attack details: {attack_details}")

...
from llm import LLM, ExposedActivationsRequest, LLMResponses, TokenSelectionMethod
...
class AutoLLM(LLM):
    ...
    def generate_responses(
        ...
        if exposed_activations_request:
            ...
            activations_list = [[torch.zeros(1) for _ in target_layers] for _ in prompts_or_embeddings]
        ...
        # Convert prompts to strings if they are embeddings
        if isinstance(prompts_or_embeddings[0], torch.Tensor):
            prompts_or_embeddings = [self.embedding_to_string(prompt) for prompt in prompts_or_embeddings]
    ...
    def generate_responses_forced(
        ...
        tokenized_chat = {k: v.to(self._model.device) for k, v in tokenized_chat.items()}
        pass
    ...
    def embedding_to_string(self, embedding: torch.Tensor) -> str:
        """Converts a "naked" embedding tensor to a prompt/response string. i.e. Does not add any special tokens or padding."""
        return self._tokenizer.decode(embedding, skip_special_tokens=True)

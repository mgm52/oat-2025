...
# %%
checkpoint_dir = "checkpoints/adversaries_obf"
batch_size = 2  #2  #4
...
#             sample_out = lora_model.generate(encoder.tokenizer([bad_prompt, bad_prompt], return_tensors="pt", padding=True, add_special_tokens=False).input_ids.to("cuda"), do_sample=True)
...
parser.add_argument('--adversary_checkpoint', type=str, help='Adversary checkpoint file')
parser.add_argument('--max_num_eval_prompts', type=int, required=True, help='Maximum number of evaluation prompts')

args = parser.parse_args()

checkpoint_dir = args.checkpoint_dir
adversary_checkpoint = args.adversary_checkpoint
max_num_eval_prompts = args.max_num_eval_prompts

if max_num_eval_prompts is None:
    raise

if adversary_checkpoint is None:
    latest_step = -1
    for filename in os.listdir(checkpoint_dir):
        if filename.startswith('checkpoint_step_') and filename.endswith('.pt'):
            ...
            if step > latest_step:
                latest_step = step
                adversary_checkpoint = filename

if adversary_checkpoint is not None:
    checkpoint_path = os.path.join(checkpoint_dir, adversary_checkpoint)
...
config = {
    ...
    "adv_checkpoint": adversary_checkpoint,
    "max_num_eval_prompts": max_num_eval_prompts,
    # "n_steps": n_steps,
    # "batch_size": batch_size,
    # "pgd_iterations": pgd_iterations,
    # "adversary_lr": adversary_lr,
...
csv_file_path = os.path.join(checkpoint_dir, 'benchmark_results.csv')
